...
-- examenj2.departamento definition
...
-- examenj2.detalles_pedidos definition
...
CREATE TABLE `detalles_pedidos` (
  ...
  KEY `detalles_pedidos_productosuc_FK` (`producto_id`,`sucid`),
  CONSTRAINT `detalles_pedidos_ibfk_1` FOREIGN KEY (`pedido_id`) REFERENCES `pedidos` (`pedido_id`),
  CONSTRAINT `detalles_pedidos_productosuc_FK` FOREIGN KEY (`producto_id`, `sucid`) REFERENCES `productosuc` (`productoid`, `sucursalid`)
) ENGINE=InnoDB;

-- examenj2.empleados definition
...
-- examenj2.productosuc definition

CREATE TABLE `productosuc` (
  `productoid` int NOT NULL,
  `sucursalid` int NOT NULL,
  PRIMARY KEY (`productoid`,`sucursalid`),
  KEY `productosuc_sucursal_FK` (`sucursalid`),
  CONSTRAINT `productosuc_productos_FK` FOREIGN KEY (`productoid`) REFERENCES `productos` (`producto_id`),
  CONSTRAINT `productosuc_sucursal_FK` FOREIGN KEY (`sucursalid`) REFERENCES `sucursal` (`id`)
) ENGINE=InnoDB;

-- examenj2.sucursal definition
...
```
INSERT INTO examenj2.pais (nombre) VALUES
('Colombia'),
('México'),
('Argentina'),
('Chile'),
...
INSERT INTO examenj2.detalles_pedidos (detalle_id, pedido_id, producto_id, cantidad, precio_unitario, sucid) VALUES
...
```
# con Base a la anterior informacion procederemos a realizar las siguientes consultas.
## Consultas JOINs (5)
```
Consulta 1: SELECT e.nombre AS empleado, s.nombre AS sucursal, d.nombre AS departamento FROM empleados AS e JOIN sucursal AS s ON e.sucursalid = s.id JOIN municipio AS m ON s.municipioid = m.id JOIN departamento AS d ON m.depid = d.id;
Consulta 2: SELECT c.nombre AS cliente, p.nombre AS país FROM clientes c JOIN municipio m ON c.municipioid = m.id JOIN departamento d ON m.depid = d.id JOIN pais p ON d.paisid = p.id;
Consulta 3: SELECT s.nombre AS sucursal_nombre, m.nombre AS municipio, d.nombre AS departamento FROM sucursal s JOIN empresa e ON s.empresaid = e.id JOIN municipio m ON s.municipioid = m.id JOIN departamento d ON m.depid = d.id WHERE e.nombre = 'RedElectro S.A.';
Consulta 4: SELECT e.nombre FROM empleados e JOIN sucursal s ON s.id = e.sucursalid JOIN municipio m ON m.id = s.municipioid;
Consulta 5: SELECT e.nombre, COUNT(em.empleado_id) AS total_empleados FROM empresa e JOIN sucursal s ON s.empresaid = e.id JOIN empleados em ON em.sucursalid = s.id GROUP BY e.nombre;
```
## Consultas FUNCIONES AGREGADAS (5)
```
Consulta 1: SELECT nombre, (SELECT AVG(e.salario) FROM sucursal s JOIN empleados e ON e.sucursalid = s.id WHERE s.municipioid = m.id) AS salario_promedio FROM municipio m WHERE (SELECT AVG(e.salario) FROM sucursal s JOIN empleados e ON e.sucursalid = s.id WHERE s.municipioid = m.id) IS NOT NULL;
Consulta 2: SELECT d.nombre, SUM(clientes_por_mun.total) AS total_clientes FROM departamento d JOIN ( SELECT m.depid, COUNT(c.cliente_id) AS total FROM municipio m JOIN clientes c ON c.municipioid = m.id GROUP BY m.id) AS clientes_por_mun ON clientes_por_mun.depid = d.id GROUP BY d.id ORDER BY total_clientes DESC LIMIT 3;
Consulta 3: SELECT puesto, cantidad FROM (SELECT puesto, COUNT(empleado_id) AS cantidad FROM empleados GROUP BY puesto) AS conteo_puestos ORDER BY cantidad DESC LIMIT 1;
Consulta 4: SELECT nombre, (SELECT SUM(em.salario) FROM sucursal s JOIN empleados em ON em.sucursalid = s.id WHERE s.empresaid = e.id) AS total_salarios FROM empresa e;
Consulta 5: SELECT m.nombre, COUNT(e.empleado_id) AS total_empleados FROM municipio m JOIN sucursal s ON s.municipioid = m.id JOIN empleados e ON e.sucursalid = s.id GROUP BY m.nombre;
```
## Subconsultas (5)
```
Consulta 1: SELECT e.nombre, e.salario FROM empleados e JOIN sucursal s ON e.sucursalid = s.id JOIN empresa em ON s.empresaid = em.id WHERE e.salario > (SELECT AVG(salario_empresarial) FROM (SELECT AVG(emp.salario) AS salario_empresarial FROM empleados emp JOIN sucursal suc ON emp.sucursalid = suc.id GROUP BY suc.empresaid));
Consulta 2: SELECT m.nombre FROM municipio m LEFT JOIN sucursal s ON m.id = s.municipioid WHERE s.id IS NULL;
Consulta 3: SELECT c.nombre FROM clientes c WHERE c.municipioid IN ( SELECT m.id FROM municipio m JOIN sucursal s ON s.municipioid = m.id LEFT JOIN empleados e ON e.sucursalid = s.id WHERE e.id IS NULL);
Consulta 4: SELECT e.nombre FROM empleados e WHERE e.salario > (SELECT MAX(emp.salario)    FROM empleados emp JOIN sucursal s ON emp.sucursalid = s.id JOIN municipio m ON s.municipioid = m.id);
Consulta 5: SELECT puesto FROM empleados GROUP BY puesto HAVING COUNT(DISTINCT sucursalid) >= 3;
```
